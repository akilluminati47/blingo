...
// Pass a `thickness` and the sheet stops being paper-thin: a back cap is sunk `thickness`
// INTO the body (the true surface sits between the two caps, so nothing floats off it).
// Purely embedded depth — no rims, no traced edges — the visible faces are unchanged.
function curvedBodyDecal(w, h, a, b, c, centerYOffset, proud = 0.015, thickness = 0) {
  ...
  for (let i = 0; i < cnt; i++) { positions.push(pos.getX(i), pos.getY(i), zs[i]); uvs.push(uv.getX(i), uv.getY(i)); }
  for (let i = 0; i < cnt; i++) { positions.push(pos.getX(i), pos.getY(i), zs[i] - thickness); uvs.push(uv.getX(i), uv.getY(i)); }
  for (let r = 0; r < 6; r++) {
...
function buildFbiBlob(faceColor, big) {
  ...
  // (thickness 0.06) so the skin's back cap is sunk INTO the skull — the head surface
  // sits between the caps and the front face is untouched (no visible edges).
  ...
  const face = new THREE.Mesh(
    curvedBodyDecal(0.62, 0.34, 0.42, 0.4, 0.4, 0.05, 0.012, 0.06),
  ...
  const back = new THREE.Mesh(
    curvedBodyDecal(0.66, 0.44, 0.55, 0.62, 0.5, 0, 0.012, 0.06),
