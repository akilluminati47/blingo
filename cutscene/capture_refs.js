...
(async function captureRefs() {
  ...
  let _origW, _origH;
  let _cullingRefs = null;
  function noFrustumCull() {
    if (_cullingRefs) return;
    _cullingRefs = [];
    scene.traverse(o => { if (o.isMesh) { _cullingRefs.push([o, o.frustumCulled]); o.frustumCulled = false; } });
  }
  function restoreCulling() {
    if (!_cullingRefs) return;
    for (const [mesh, v] of _cullingRefs) mesh.frustumCulled = v;
    _cullingRefs = null;
  }
  function snap(name) {
